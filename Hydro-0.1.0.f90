PROGRAM HydroSim
  ...
  DOUBLE PRECISION :: t, G, pi, h, dist, temperature, dt, kb, dx, dy, dz, twrite, tcount, tlim, q
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: ranPos, ranVel, m, kern0, kern1, dens, press
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: r, v, a, kern, kerndiv, dists
  ...
  OPEN(11, file='time.txt', status='replace')
...
10   CALL random_NUMBER(ranPos)
  ...
  DO
     ...
     DO i=1,n ! Finding nearest neighbours, densities, pressures and kernels for each particle
        ...
        h = nears(32,i)/2. ! 2h is the distance to the 32nd nearest neighbour
        CALL kernel(kernTot, ndims, h, kern0(:), kern1(:)) ! Fills the kern0 and kern1 arrays with appropriate values
        ...
        DO j=1,nneighs ! This fills the kern and kerndiv arrays with approximate values, then works out the density at particle i by summing over contributions from the nearest neighbours
           ...
           ELSE ! Assumes qint is not greater than 1000
              kern(j,i) = kern0(qint)
     ...
     DO i=1,n ! Acceleration needs to be calculated after everything else as the contribution to the acceleration of particle i from particle j depends on the pressures and densities of both i and j. Acceleration components can be found by multiplying the acceleration by dx/a, where a is the total separation and dx is the separation along a given axis
        DO j=1, nneighs
           ...
           a(1,i) = a(1,i) - (m(nears(j,i)) * ((press(nears(j,i))/(dens(nears(j,i))**2)) + (press(i)/(dens(i)**2))) * kerndiv(j,i) * (dx/dist))
           a(2,i) = a(2,i) - (m(nears(j,i)) * ((press(nears(j,i))/(dens(nears(j,i))**2)) + (press(i)/(dens(i)**2))) * kerndiv(j,i) * (dy/dist))
           a(3,i) = a(3,i) - (m(nears(j,i)) * ((press(nears(j,i))/(dens(nears(j,i))**2)) + (press(i)/(dens(i)**2))) * kerndiv(j,i) * (dz/dist))
        END DO
     END DO
