PROGRAM ShockTube
  ...
  DOUBLE PRECISION :: tempDist, q, pi, kb, adiabrat, etasq, vdotr, muab, avsndspd, avdens, tempAcc, t, tcount, dt, tlim, twrite, viscalpha, viscbeta, ran3, thruDist, ptrop, kptrop, thrudr, hmean, hdiff
  ...
  OPEN(13, file='hvalues.txt', status='replace')
  ...
  recomplim = 1 ! Number of timesteps between each time the nbneighs nearest neighbours should be identified
  ...
  allDists = 0.
  thruDist = 0.
  ptrop = 1./((LOG(10.)/LOG(8.)) - 1.) ! SHOULD produce the desired initial left-right pressure ratio
  kptrop = 1.
  thrudr = 0.
  hmean = 0.
  hdiff = 0.
  ...
  DO i=1,n ! Placing particles and setting up IDs array
     ...
     DO k=1,ndims
        ...
        IF (k==1) THEN
           IF (i<=n/9) THEN
              r(k,i) = (ranPos(k) + 1.) / 2. ! Forces x-coordinate into right half of tube (between x=0.5 and x=1)
           ELSE
              r(k,i) = ranPos(k) / 2. ! Forces x-coordinate into left half of tube
           END IF
        ELSE
           r(k,i) = ranPos(k) * 0.1 ! Forces y- and z-coordinates to between 0 and 0.1
        END IF
  ...
  DO
        ...
        DO i=1,n
           DO j=1,n
              ...
              DO k=1,ndims
                 ...
                 IF (k==1) THEN
                    thruDist = thruDist + ((r(k,j) - r(k,i))**2)
                 ELSE
                    thruDist = thruDist + ((r(k,i) + (0.1 - r(k,j)))**2) ! Through-boundary distance for y- and z-components
                 END IF
              END DO
              IF (thruDist < tempDist) THEN
                 allDists(j) = SQRT(thruDist)
              ELSE
           ...
           DO j=1,nbneighs
              ...
              DO k=1,ndims
                 ...
                 IF (k==1) THEN
                    thruDist = thruDist + ((r(k,j) - r(k,i))**2)
                 ELSE
                    thruDist = thruDist + ((r(k,i) + (0.1 - r(k,j)))**2)
                 END IF
              END DO
              IF (thruDist < tempDist) THEN
                 nearDists(j) = SQRT(thruDist)
              ELSE
     ...
     DO i=1,n ! Densities, pressures, sound speeds and kernels for each particle
        ...
        press(i) = kptrop * (dens(i)**(1.+(1./ptrop)))
        temps(i) = (m(i) * press(i)) / (kb * dens(i))
     ...
     DO i=1,n ! Artificial Viscosity
        ...
        DO j=1,nneighs
           ...
           DO k=1,ndims
              ...
              ELSE
                 tempDist = r(k,i) - r(k,nearIDs(j,i))
                 thruDist = (0.1 - r(k,i)) + r(k,nearIDs(j,i))
                 IF ((tempDist**2) >= (thruDist**2)) THEN ! Used this condition as the ABS function does not allow the inputs to be DOUBLE PRECISION. If x^2 >= y^2 then it follows that ABS(x) >= ABS(y), so the conditions are equivalent
                    vdotr = vdotr + ((v(k,i) - v(k,nearIDs(j,i))) * thruDist)
                 ELSE
                    vdotr = vdotr + ((v(k,i) - v(k,nearIDs(j,i))) * tempDist)
                 END IF
              END IF   
           END DO
           IF (vdotr < 0.) THEN              
     ...
     DO i=1,n ! Accceleration needs to be calculated after everything else as the contribution to the acceleration of particle i from neighbour j depends on the pressures and densities at both i and j. Components of acceleration can be found by multiplying the acceleration by dr(k)/a, where a is the total separation and dr(k) is the separation along a given axis k
        ...
        DO j=1, nneighs
           ...
           DO k=1,ndims
              ...
              IF (k .ne. 1) THEN
                 thrudr = (0.1 - r(k,i)) + r(k,nearIDs(j,i))
                 IF ((thrudr**2) < (dr(k)**2)) THEN
                    dr(k) = thrudr
                 END IF
              END IF
     ...
     IF (counter==1) THEN ! Writing the initial simulation state to a file
        WRITE(7,*) r(1,:)
        WRITE(8,*) v(:,:)
        WRITE(9,*) dens(:)
        WRITE(10,*) press(:)
     ...
     DO i=1,n ! Advancing the simulation       
        DO k=1,ndims
           ...
           IF (k==1) THEN
              IF (r(k,i) > 1.) THEN
                 r(k,i) = 2. - r(k,i) ! (dist = r(1,i) - 1.), then new x-coordinate is (R(1,i) = 1. - dist)
                 v(k,i) = v(k,i) * (-1.)
              ELSE IF (r(k,i) < 0.) THEN
                 r(k,i) = r(k,i) * (-1.) ! Distance from x=0 is preserved when multiplying by -1
                 v(k,i) = v(k,i) * (-1.)
              END IF
           ELSE
              IF (r(k,i) > 0.1) THEN
20               r(k,i) = r(k,i) - 0.1
                 IF (r(k,i) > 0.1) THEN
                    GOTO 20 ! Needed just in case particle is still outside of the boundary
                 END IF
              ELSE IF (r(k,i) < 0.) THEN
21               r(k,i) = r(k,i) + 0.1
                 IF (r(k,i) < 0.) THEN
