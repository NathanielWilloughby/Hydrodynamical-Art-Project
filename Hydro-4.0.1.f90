SUBROUTINE HydroSim(n, ndims, rin, vin, tempin, tlim, dt, cin, press, dens, temps, r, v, counter)
  ...
  DOUBLE PRECISION, INTENT(IN) :: rin(1:ndims,1:n), vin(1:ndims,1:n), tempin(1:n), tlim, dt
  DOUBLE PRECISION, INTENT(OUT) :: press(1:n), dens(1:n), temps(1:n), r(1:ndims,1:n), v(1:ndims,1:n)
  DOUBLE PRECISION :: tempDist, q, pi, kb, adiabrat, etasq, vdotr, muab, avsndspd, avdens, tempAcc, t, viscalpha, viscbeta
  ...
  DO i=1,nbneighs ! Setting up an array of the first 100 natural numbers. These correspond to indices for values in the nearIDs array, and will be constantly re-ordered throughout according to distance from a given particle
     bneighsIDs(i) = i
  END DO
  ...
  DO
     ...
     counter = counter + 1 ! This goes at the start of the loop because I want counter to be 1 during the first timestep
     ...
     IF (recomp==recomplim) THEN ! This IF-ELSE statement finds the IDs of the nearest neighbours to particle i, as well as their distances to particle i
        ...
        !$OMP PARALLEL
        !$OMP DO PRIVATE(tempDist, allDists, IDs, j, k)
        DO i=1,n
           DO j=1,n
              tempDist = 0.
              DO k=1,ndims
     ...
     ELSE
        ...
        !$OMP PARALLEL
        !$OMP DO PRIVATE(tempDist, nearDists, bneighsIDs, tempIDs, j, k)
        DO i=1,n
           DO j=1,nbneighs
              tempDist = 0.
              DO k=1,ndims
...
SUBROUTINE kernel(kernTot, dims, hval, kern0, kern1)
  ...
  DOUBLE PRECISION :: qval, pi, prefac1, prefac2
