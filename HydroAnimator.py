...
if (dmax/dmin) > 1e5:
    # Would be better to use a logarithmic scale if the range is very large
    dnorm = cols.LogNorm(dmin, dmax)
else:
    dnorm = cols.Normalize(dmin, dmax)

if (tmax/tmin) > 1e5:
    tnorm = cols.LogNorm(tmin, tmax)
else:
    tnorm = cols.Normalize(tmin, tmax)

if (vmax/vmin) > 1e5:
    vnorm = cols.LogNorm(vmin, vmax)
else:
