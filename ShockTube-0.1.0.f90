PROGRAM HydroSim
  ...
  DO i=1,n ! Setting up Initial Conditions and the IDs array
     IF (i<=n/8) THEN ! 1/8th of the particles will be in the right half of the tube so that the correct initial density profile is obtained
        call random_NUMBER(ranPos)
        r(1,i) = (ranPos(1) + 1.) / 2. ! Pushes the x-coordinate to between 1 and 2, then divides it by 2. This ensures the particle is in the right half of the tube
        r(2,i) = ranPos(2) * 0.1
        r(3,i) = ranPos(3) * 0.1
     ELSE
        call random_NUMBER(ranPos)
        r(1,i) = ranPos(1) / 2. ! Forces the x-coordinate between 0 and 0.5
        r(2,i) = ranPos(2) * 0.1
        r(3,i) = ranPos(3) * 0.1
     END IF
     call random_NUMBER(ranVel) ! No restrictions on velocity distributions on either side
  ...
  DO
     ...
     DO i=1,n ! Finding nearest neighbours, densities, pressures, sound speeds and kernels for each particle
        DO j=1,n ! Finding distances from particle i to each particle
           ...
           dist = SQRT(((r(1,j) - r(1,i))**2) + ((r(2,i) + (0.1 - r(2,j)))**2) + ((r(3,i) + (0.1 - r(3,j)))**2)) ! Since the y- and z- boundaries are periodic, the distance could also be found in the opposite direction. Here the distance along the y- and z-axes would be the sum of the distance of particle i from the y=0 or z=0 boundary and the distance of particle j from the y=0.1 or z=0.1 boundary
           IF (dist < dists(j,i)) THEN
              dists(j,i) = dist
           END IF
     ...
     DO i=1,n ! Acceleration needs to be calculated after everything else as the contribution to the acceleration of particle i from particle j depends on the pressures and densities of both i and j. Acceleration components can be found by multiplying the acceleration by dx/a, where a is the total separation and dx is the separation along a given axis
        DO j=1, nneighs
           ...
           dist = r(2,i) + (0.1 - r(2,nears(j,i))) ! Through-boundary y-distance
           IF (dist < dy) THEN
              dy = dist
           END IF
           dz = r(3,nears(j,i)) - r(3,i) 
           dist = r(3,i) + (0.1 - r(3,nears(j,i))) ! Through-boundary z-distance
           IF (dist < dz) THEN
              dz = dist
           END IF
     ...
     DO i=1,n ! Advancing the simulation
        r(:,i) = r(:,i) + (v(:,i) * dt)
        v(:,i) = v(:,i) + (a(:,i) * dt)
        IF (r(1,i) > 1.) THEN ! Bouncing particles off the x-wall by forcing out of range x-coordinates to be the same distance from the boundary, only inside the tube instead of outside of it. x-velocities also need to be multiplied by -1 to simulate the change in direction due to rebounding off the wall
           r(1,i) = 2. - r(1,i) ! This comes from (dist = r(1,i) - 1.), then finding the new x-coordinate as (R(1,i) = 1. - dist)
           v(1,i) = v(1,i) * -1.
        ELSE IF (r(1,i) < 0.) THEN
           r(1,i) = r(1,i) * -1. ! Here the particle can be forced back into the tube by simply multiplying its x-coordinate by -1, which preserves the same distance from x=0
           v(1,i) = v(1,i) * -1.
        END IF

11      IF (r(2,i) > 0.1) THEN ! Creating a periodic boundary parallel to the y-axis. Velocities do not need to be changed if the particle crosses this boundary as it will not have bounced off of it
           r(2,i) = r(2,i) - 0.1
           GOTO 11 ! This is needed just in case the particle is still outside of the boundary
12      ELSE IF (r(2,i) < 0.) THEN
           r(2,i) = r(2,i) + 0.1
           GOTO 12 ! Using GOTO 12 instead of GOTO 11 because I don't want to risk getting the program stuck on this loop. Also r(2,i) = r(2,i) + 0.1 shouldn't place the particle above y=0.1 if it started below y=0 anyway
        END IF

13      IF (r(3,i) > 0.1) THEN ! z-axis periodic boundary
           r(3,i) = r(3,i) - 0.1
           GOTO 13
14      ELSE IF (r(3,i) < 0.) THEN
           r(3,i) = r(3,i) + 0.1
           GOTO 14
        END IF
